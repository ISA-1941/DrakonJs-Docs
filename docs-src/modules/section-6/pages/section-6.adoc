...
As an example, consider constructing a hash table of size `m = 16` for a collection of 16 key-value pairs, where the key is an identification number and the value is a full name (Table 2). 

[.text-right]
Table 2. The collection of 16 key-value pairs
