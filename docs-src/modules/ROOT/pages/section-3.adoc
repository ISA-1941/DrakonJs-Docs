...
Sequential access means that a group of elements is accessed in a predefined ordered sequence. An example of sequential access is a singly linked list. Direct access, by contrast, means that individual elements of the data structure are accessed directly by providing a unique address for each element.
...
For example, the collection of integers [31, 26, 19, 24, 32, ....35, 41, 14, 33, 29] forms an array (Figure 3.1).

[.text-center]
.Figure 10.1. Memory hierarchy of a modern computer
image::section_3/Fig10_1_MMO_darkgray.svg[width=100%, link=self, window="_blank"]

[.text-center]
.Figure 3.1. One-Dimensional Array
image::section_3/Fig3_1_Array.svg[width=100%,link=self, window="_blank"]
...
image::section_3/Fig3_2_SliceNew.svg[width=100%, link=self, window="_blank"]
...
[source, javascript]
----
let sliceA = parentSlice.slice(0, 4); // sliceA contains the first 4 elements (indices 0–3)
let sliceB = parentSlice.slice(12, 14); // sliceB contains 2 elements (indices 12–13)
----

In JavaScript, sliceA will contain elements from index 0 to 3, and sliceB will contain elements from index 12 to 13.
...
[.text-center]
.Figure 3.3. Map
image::section_3/Fig3_3_MapNew.svg[width=75  %, align=center, link=self, window="_blank"]
...
image::section_3/Fig3_4_LLNew.svg[width=100%, link=self, window="_blank"]

a) Data (which can be any primitive or complex data type)
b) A reference (pointer) to the next node in the list
...
a) A reference to the first node (head)
b) A counter (size) to track the number of elements
...
A stack is an abstract data type that contains elements with two basic operations: push, which adds an item to the collection, and pop, which deletes the last item added. From a technological point of view, a stack is memory where data values are loaded and retrieved according to the "Last-In-First-Out" (LIFO) strategy. Data enters the stack from only one side, called the top of the stack (Figure 3.5).
...
a) enqueue(item) – Adds an element to the back of the queue
b) dequeue() – Removes an element from the front of the queue
...
A binary tree in JavaScript can be represented as a set of linked node objects. Each node contains a value and two references: one to the left child and one to the right child. If a child does not exist, the corresponding reference is null. The entire tree is accessed through a reference to its root node.
...
This factory-style representation avoids the use of class and this, while keeping the structure simple and explicit. It is convenient for describing algorithms in DRAKON diagrams: operations such as insertion, search, deletion, and traversal can be shown as manipulations of ordinary objects and their left and right links. JavaScript code then directly follows the same logic, creating nodes with a factory function and updating references between them.

== 3.3. Summary

This section covered the practical realization of fundamental Abstract Data Types (ADTs) in JavaScript, bridging theoretical concepts with concrete implementation techniques. 

We examined both linear and non-linear data structures:
* **Dynamic Arrays and Sets:** Leveraging JavaScript's built-in capabilities and performance tradeoffs.
* **Linked Lists (Singly and Doubly):** Implementing pointer-based structures manually using factory functions and reference manipulation.
* **Stacks and Queues:** Enforcing LIFO and FIFO behavior for operational constraints and algorithmic tasks such as parsing and state traversal.
* **Binary Trees:** Representing hierarchical structures via linked node objects.

By relying on explicit factory functions rather than class-based syntax, these implementations maintain a direct correspondence with visual DRAKON diagrams. This hybrid methodology lays a solid foundation for evaluating memory dynamics, pointer operations, and algorithmic time complexity in subsequent sections.