:imagesdir: docs-src/modules/section-2/assets/images
[.text-justify]
:figure-caption!:
...
This section assumes prior familiarity with basic programming concepts in at least one modern high-level language. At the same time, the description of JavaScript in this book is limited to the constructs necessary for understanding data structure algorithms.
...
The basic structure of a JavaScript program can be organized as follows:
...
[source,javascript]
----
console.log("Hello %d", 23); // Equivalent to Go's fmt.Printf("Hello %d", 23)
console.log("Name: %s, Salary: %d", "Smith", 32000);
----

c) Using Template Literals (Recommended)

Template strings provide an easy way to interpolate variables and expressions into strings.
...
The function prompt() always returns a string, so numbers must be converted using parseInt() or parseFloat():
...
A representation of this construct in DRAKON technology is shown in the next section.
...
b) while Loop (Preconditioned Loop)
...
[options="header"]
|===
| JavaScript Construct | Usage
| for-of | Iterating over arrays (values only)
| forEach() | Iterating over arrays with index and value
...
[source,javascript]
----
let bigNumber = 9007199254740993n; // BigInt type (suffix 'n' is required)
----
...
**Summary of Composite Data Types:**
...
Inheritance, encapsulation, and polymorphism are the three pillars of object-oriented programming (OOP) that play a key role in modern programming languages. They help create more modular, flexible, and maintainable code.
...
Factory functions provide a flexible and powerful way to implement OOP principles in JavaScript. Using factory functions helps to prevent the problems of classical prototypical inheritance. With the help of factory functions, you can use object composition, which in many cases is a more flexible solution than inheritance.
...
This section provided a comprehensive overview of the JavaScript programming language and the DRAKON visual algorithmic language, establishing the practical foundation for implementing data structures and algorithms in this book.

JavaScript’s core syntax, dynamic typing, flexible object model, and functional programming capabilities (such as first-class functions and closures) make it a versatile tool for algorithm design. We explored both class-based and object-composition paradigms, alongside essential control structures and asynchronous primitives (`Promises`, `async/await`). 

In parallel, the DRAKON language introduces a visual, structured methodology for algorithm specification, minimizing cognitive load and preventing common flow-control errors. Together with JavaScript, this hybrid approach bridges high-level algorithmic reasoning with robust, modern software implementation.

With the core language mechanics and visual modeling principles established, we are fully prepared to proceed to concrete data structure implementations, algorithmic analysis, and performance optimization in the upcoming sections.