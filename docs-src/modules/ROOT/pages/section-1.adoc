:imagesdir: docs-src/modules/section-1/assets/images
[.text-justify]
:figure-caption!:
...
image::section_1/Fig1_1_class.svg[width=200%, link=self, window="_blank"]
...
image::section_1/Fig1_2_Array.svg[width=50%, link=self, window="_blank"]
...
A set is a linear abstract data type that stores a collection of unique values. While it shares similarities with arrays in terms of being a sequential collection, its defining characteristic is the uniqueness invariant — it automatically prevents the storage of duplicate elements.

`Characteristics of Sets:`
    Unique Elements: Each value can appear only once in the collection.
    Ordered Insertion: In modern implementations (like JavaScript's Set), elements are iterated in the order they were added.
    Dynamic Size: Unlike traditional arrays, sets can grow dynamically as new unique values are inserted.

`Main Operations on Sets:`
    Add(value): Inserts a new element if it does not already exist.
    Has(value): A highly optimized check for the existence of an element.
    Delete(value): Removes a specific element from the collection.
    Size: Returns the total count of unique elements.

`Advantages of Sets:`
    Efficient Deduplication: Eliminates the need for manual checks when collecting unique data (e.g., generating unique keys for a Red-Black Tree).
    Fast Membership Testing: Searching for an element is generally faster than in a standard array or linked list.

`Limitations of Sets:`
    No Indexed Access: Unlike arrays, you cannot typically access the "third" element directly via set[2]; you must iterate through the collection.
    Single Type of Logic: Only stores values, not the key-value pairs found in associative arrays.

🔹 Applications of Sets:

    Data Preprocessing: Ensuring that input data (like keys for tree structures) contains no duplicates before processing.
    Filtering: Removing redundant entries from a stream of information.
    Tracking Visited Nodes: Used in algorithms like Breadth-First Search (BFS) to avoid infinite loops by storing already processed vertices.
...
image::section_1/Fig1_3_LL.svg[scalewidth=25%, link=self, window="_blank"]
...
image::section_1/Fig1_4_Stack.svg[width=75%, link=self, window="_blank"]
...
image::section_1/Fig1_5_Queue.svg[width=75%, link=self, window="_blank"]
...
=== 1.3.1. Associative arrays

In computer science, an associative array is an abstract data type that stores a collection of (`key, value`) pairs, such that each possible key appears at most once in the collection. 

Unlike a regular array an associative ones:

- uses keys (which can be strings, numbers, or other immutable data types) to access elements.

- he order of elements in an associative array is usually not guaranteed (depending on the implementation of the programming language). The mapping of keys to values ​​is important, not their sequence.

- designed to store sets of data where each element is identified by a unique name (key). This makes the code more readable and allows easy access to values ​​by their meaningful name.

An associative array that follows these rules:

- any entry's value can be accessed using its key.

- the same value can exist under multiple keys.

- a single key can only map to one unique value.

image::_images/Fig1_6_AssociativeArray.jpg[width=75]

Associative array provides basic operations: 

- insert(key, value) - adds a key-value pair;

- find(key) value - returns the value of the key (or an error if the key is not present);

- remove(key) - deletes the key pair.

contains(key)   bool - checks for a key.

size()   int - returns the number of elements.

The practical implementation of the basic functions of associative arrays can be done using various approaches: hashing, trees, lists of key-value pairs. The most common, due to its efficiency, is hashing as a way to implement these operations through a hash table. The etymology of the words "hash" and "hashing" is related to the French verb "hacher", which means "chop, shred". Hash functions "cut" the input data into parts and "mix" them to obtain an output value of a fixed length.

In theory, it is important to distinguish between the interface (what the operation does) and the implementation (how it does it). The first description of hash functions as a mathematical function that transforms input data of arbitrary size (called a "key" or "message") into output data of fixed size (called a "hash value") was given by Hans Peter Luhn (1953). Donald Knuth (1963) and Wesley Peterson (1968) systematized hashing as an abstract operation on data. 

Algorithms for practical implementation of basic functions of associative arrays (hash tables) are presented in Section 6.
...
image::section_1/Fig1_6_Tree.svg[width=50%, link=self, window="_blank"]
...
image::section_1/Fig1_7_Graph.svg[width=100%, link=self, window="_blank"]
