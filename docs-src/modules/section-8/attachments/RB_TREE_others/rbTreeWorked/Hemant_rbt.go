...
func (rbt *RBTree) findNode(data int) *Node {
	// item 129
	curr := rbt.root
	for {
		// item 130
...
function findNode(node, value) {
    while (node !== null) {
        if (value === node.value) {
            ...
            throw new Error("Unexpected comparison in findNode");
        }
    }
    return null;
}
...
func (rbt *RBTree) deleteNode(node *Node, key int) {
	...
	if z == rbt.nullNode {
		// item 348
		fmt.Println("Couldn't findNode key in the tree")
		return
	} else {
...
func (rbt *RBTree) fixDoubleBlack(x *Node) {
	// item 209
	if x == rbt.root {
		// item 212
		return
	...
	if sib == rbt.nullNode {
		...
		if sib.colour == true {
			...
			if sib.left.colour == true || sib.right.colour == true {
				...
				if sib.parent.left == sib {
					// item 235
					if sib.left != rbt.nullNode && sib.left.colour == true {
						// item 238
...
func (rbt *RBTree) fixRedRed(x *Node) {
	// item 159
	if x == rbt.root {
		// item 162
		x.colour = false
	...
	// item 164
	if x.parent == rbt.nullNode ||
		x.parent.parent == rbt.nullNode {
		// item 167
		return
	} else {
...
function fixRedRed(tree, x) {
    ...
    // item 393
    const a = parent === GP.left && x === parent.left;
    const b = parent === GP.left && x === parent.right;
    const c = parent === GP.right && x === parent.left;
    const d = parent === GP.right && x === parent.right;
    ...
    if (a) {
        // item 194
        mid = rightRotate(tree, GP);
    } 
    // item 1870002
    else if (b) {
        // item 197
        mid = leftRightRotate(tree, GP);
    }
    // item 1870003
    else if (c) {
        // item 198
        mid = rightLeftRotate(tree, GP);
    }
    // item 1870004
    else if (d) {
        // item 199
        mid = leftRotate(tree, GP);
    } else {
        // item 397
        console.log(" Empty ");
    }
