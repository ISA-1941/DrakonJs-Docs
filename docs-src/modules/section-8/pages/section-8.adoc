...
Node includes the following fields: `data` - node numeric value,  `colour` - node colour type bool: `red node` - TRUE; `black node` - false, `left`, `right` and `parent` nodes. In turn RBTree structure represents the reference type variable, that store objects containing not the "value" itself but a reference to the memory area where this object is stored (Table. 8.?).

[cols="1,1", options="header", frame=none, grid=none, align=center]
|===
| *Function: createNode* | *Function: createTree*
