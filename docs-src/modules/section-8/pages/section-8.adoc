...
The term nodeImage denotes the local configuration of nodes within a tree-cluster, including their colors and relative positions (left or right).
...
Each possible configuration (or nodeImage) corresponds to a typical balancing case, which determines the required sequence of operations: recoloring and/or rotation.
All canonical nodeImage configurations representing possible red and black node arrangements are summarized in Table 8.3, which shows how each type of violation is detected and corrected to preserve all red-black properties.
:!table-caption:
...
This example demonstrates how local rotations and recoloring maintain global balance. Despite multiple insertions, the tree height grows logarithmically, illustrating the efficiency of the red-black approach.

==== 8.4.3. Deleting a Node from a red-black tree

Removing a node from a red-black tree is a complex process because it depends on the location of the node, the presence of children, and the colour of the nodes involved. To understand the balancing operations, we first introduce the standard notation used throughout this section (Figure 8.37):
...
s.left, s.right: The left and right nephews (children) of the sibling s.

image::{imagesdir}/Fig8_37_Term.jpg[width=100%]
...
transformations of the tree structure must maintain its properties. This
algorithm can be divided into three stages.

*Stage 1: Standard BST Deletion*
First, the node is removed using the standard binary search tree deletion logic. The outcome of this step determines the necessity of further rebalancing.

- If the deleted node was red, no properties are violated, and the process is complete.

- If the deleted node was black, it may create a "double-black" deficit in the tree, which must be resolved.
