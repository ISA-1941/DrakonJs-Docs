...
[loweralpha]
     1. Deleting a Red Node: This is the simplest case. A red node does not contribute to the black height of any path. Simply removing it does not violate any RBT properties, and the tree remains balanced. 

[loweralpha]
     2. Deleting a Black Node: This is the more complex case. Removing a black node violates the RBT property that requires all paths from the root to the leaf nodes to contain the same number of black nodes (the black height). Restoring this balance requires an iterative rebalancing process.
