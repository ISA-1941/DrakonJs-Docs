...
ancestor (parent of parent), U (Uncle) - uncle of node x, S - siblings (Figure 8.31).

image::{imagesdir}/Fig8_31_family.jpg[width=75%]

[.text-center]
Figure 8.31. Designation of the degree of "kinship" of RB Tree nodes (x, P, G, U, S)  
...
image::{imagesdir}/Fig8_32_rbInsert.png[width=75%, link="{imagesdir}/Fig8_32_rbInsert.svg", window="_blank"]
...
The term `nodeImage` denotes the local configuration of nodes within a tree-cluster,
including their colors and relative positions (left or right).
When a new element is inserted, it is initially assigned the red color.
To satisfy the first two rules, it is sufficient to repaint the new vertex as needed.
After each insertion, all red-black tree properties must be verified.
If at least one property is violated, rotation and recoloring operations are performed.
The different `nodeImage` representing possible configurations of red and black nodes are summarized in Table 8.3. 

:!table-caption:
.Table 8.3 — NodeImage for Red-Black Tree Insertion Cases
|===
...
| Recolor P and U to black (P:B, U:B), G to red (G:R); set x = G (propagate upward). Note: After this operation, the violation (two consecutive red nodes) may propagate up two levels. The algorithm must continue by     |treating G as the new current node x (i.e., it performs a recursive ascent). If G is the root, it will remain red after recoloring, and the algorithm will ultimately repaint the root to black in its final step.
...
The different `nodeImage` representing possible configurations of red and black nodea are summarized in Table 8.4.

image::{imagesdir}/Case_1.png[width=100%]

[cols="1,1,2,2,2", options="header"]
|===
| Case | NodeImage & Path | Violation | Fix Strategy | Rotation Type
| Red Uncle
|
x:R, x←P, P:R, P←G, G:B, U:R
|
...
U:B
|
x and P are both red and form a left-left line; uncle is black.
|
...
U:B
|
x and P are both red and form a right-right line; uncle is black.
|
...
U:B
|
x and P are both red and form a left-right line; uncle is black.
|

Left-rotate about P.

Now x and P form a left-left line.

Apply Case 1 (L-L) strategy.
|
...
U:B
|
x and P are both red and form a right-left line; uncle is black.
|

Right-rotate about P.

Now x and P form a right-right line.

Apply Case 2 (R-R) strategy.
|
...
Explanation of the Types of Rotations Used in the Balancing Process 
...
[.text-center]
Figure 8.35. DRAKON-diagrams of the red-black tree cluster node rotation algorithms
...
`[11,12,13,14,15,16,17,18,19]` (Figure 8.36):

image::{imagesdir}/Fig8_36_rbProcess.jpg[width=75%]

[.text-center]
Figure 8.36 Process for Balancing the Red-Black Tree
...
image::{imagesdir}/Fig8_38a_mindMap_deleteNode.png[width=100%, link="{imagesdir}/Fig8_38a_mindMap_deleteNode.svg", window="_blank"]

//image::{imagesdir}/Fig8_38b_deleteNode.png[width=75%]
image::{imagesdir}/Fig8_38b_deleteNode.png[width=100%, link="{imagesdir}/Fig8_38b_deleteNode.svg", window="_blank"]
...
Replacing a node in a red-black tree using `joinParentChild` is shown on Figure 8.40:
[caption="Figure 8.40", align="center"]

image::{imagesdir}/Fig8_40_joinPC.png[width=80%]

[.text-center]
Figure 40. Replacing a node in a red-black tree
...
further changes to the tree structure (fig.8.41).

image::{imagesdir}/Fig8_41_delRed.png[width=80%]

[.text-center]
Figure 8.41. Deletion of red node (20)
...
the red-black wood are preserved. The Drakon-diagrams algorithm of this
process is shown in Fig.8.42.

image::{imagesdir}/Fig8_42a_fixDoubleBlack.png[width=80%]
...
[.text-center]
Figure 8.42. Drakon-diagram of the node structure correction algorithm when removing "double black node"
